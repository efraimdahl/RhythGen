...
def abcdur (nx, divs, uL):      # convert an musicXML duration d to abc units with L:1/uL
    ...
    if den > MINIMUM_NOTE_VALUE:                # limit the denominator to a maximum of 64
        x = float (num) / den; n = math.floor (x);  # when just above an integer n
        if x - n < 0.1 * x: num, den = n, 1;        # round to n
        num16 = float(MINIMUM_NOTE_VALUE) * num / den + 1.0e-15           # to get Python2 behaviour of round
        num, den = simplify (int (round (num16)), MINIMUM_NOTE_VALUE)
    if num == 1:
        if   den == 1: dabc = ''
        elif den == 2: dabc = '/'
        else:          dabc = '/%d' % den
...
def chkbug (dt, m):
    if dt > m.divs / (MINIMUM_NOTE_VALUE//4): return 1   # duration should be > 1/MINIMUM_NOTE_VALUE note
    info ('MuseScore bug: incorrect duration, smaller then 1/MINIMUM_NOTE_VALUE! in measure %d, part %d' % (m.ixm, m.ixp))
    return 0
