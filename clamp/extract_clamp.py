input_dirs = [
    "../evaluation_data/outputs/base",
]
             
output_dirs = [
    "../evaluation_data/clamp_embeddings/base",
]
...
for input_dir in input_dirs:
    ...
    """
    """
    with open(f"logs/files_extract_clamp2_{dirname}.json", "w", encoding="utf-8") as f:
        json.dump(files, f)
    random.shuffle(files)
    with open(f"logs/files_shuffle_extract_clamp2_{dirname}.json", "w", encoding="utf-8") as f:
        json.dump(files, f) 
    file_met.append(files)
...
for input_dir in input_dirs:
    ...
    with open(f"logs/files_shuffle_extract_clamp2_{dirname}.json", "r", encoding="utf-8") as f:
        files = json.load(f)
        file_met.append(files)


for input_dir,output_dir,files in zip(input_dirs,output_dirs,file_met):
    dirname = input_dir.split("/")[-1] 

    print(f"Extracting {len(files)} files from {input_dir} to {output_dir}")
    # process the files
    process_directory(input_dir, output_dir, files)

    with open("logs/log_extract_clamp_{dirname}.txt", "a", encoding="utf-8") as f:
